...
import { Search } from 'global/interfaces/search';
...
const URLS = {
  ...
  SEARCH: `search.json?key=${API_KEYS.WEATHER_API_KEY_VALUE}`,
};
...
export const weatherApi = createApi({
  ...
  endpoints: builder => ({
    ...
    search: builder.query<Search, string | number>({
      query: param => `${URLS.SEARCH}&q=${param}`,
    }),
  }),
});
